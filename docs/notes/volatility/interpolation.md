...
\[
w(k,T) = \sigma_{\mathrm{impl}}(k,T)^2\,T.
\]
...
* enforce **convexity** by requiring the spline’s second derivative \(\ge 0\) on a grid,
* enforce monotonicity (first derivative \(\le 0\)).
...
* **Piecewise-linear density** → \(C(K)\) becomes piecewise-cubic (but with guaranteed \(q \ge 0\)).
...
- Crépey, S. (2008). Tikhonov regularization.
