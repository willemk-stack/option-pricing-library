...
[Review Heston model comparison](heston_model_comparison.md){ .md-button }
...
| The library handles real provider data, not only synthetic fixtures | [Real-market provider evidence](provider_market_evidence.md) | Local provider run artifacts under `out/marketdata-live/` | Shows sanitized provider-backed ingestion, normalization, quote cleaning, policy capture, model-ready preparation, and Heston fit status without publishing raw provider payloads. |
...
5. For production-shaped marketdata questions, review [Real-market provider evidence](provider_market_evidence.md) after the deterministic proof path.
